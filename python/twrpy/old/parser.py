...
from twrpy.old.nodes import BaseNode, RecordNode, BoolNode, Int8Node, Int16Node, Int32Node, Int64Node,\
    ...
    RecordArrayNode, BoolArrayNode, Int8ArrayNode, Int16ArrayNode, Int32ArrayNode, Int64ArrayNode,\
    ...
    OptimizedBoolNode, OptimizedIntNode, OptimizedUIntNode, OptimizedFloatNode, OptimizedBoolArrayNode,OptimizedIntArrayNode, OptimizedUIntArrayNode
...
class Parser:
    ...
    def get_footer(cls,reader,header):
        ...
        if header['magic_number'] in [43978,43983]: # abca, abcf
            ...
            for i in range(ustring_size):
                ca_unicode = reader.read_ca_unicode_value()
                ex_values[reader.read_uint32_value()] = ca_unicode
    ...
    def read_value_node(cls,reader,codec,code):
        if code == TypeCodes.BOOL:
            ...
            return BaseNode
        elif code in [TypeCodes.BOOL_TRUE, TypeCodes.BOOL_FALSE]:
            return OptimizedBoolNode(reader,codec).get_value(code)
        elif code in [TypeCodes.INT32_ZERO, TypeCodes.INT32_BYTE, TypeCodes.INT32_SHORT, TypeCodes.INT32_24BIT]:
            return OptimizedIntNode(reader,codec).get_value(code)
        elif code in [TypeCodes.UINT32_ZERO, TypeCodes.UINT32_BYTE, TypeCodes.UINT32_SHORT, TypeCodes.UINT32_24BIT]:
            return OptimizedUIntNode(reader,codec).get_value(code)
        elif code == TypeCodes.FLOAT32_ZERO:
            return OptimizedFloatNode(reader,codec).get_value(code)
        else:
            raise
    ...
    def read_array_node(cls,reader,codec,code):
        ...
        elif code in [TypeCodes.BOOL_TRUE_ARRAY,TypeCodes.BOOL_FALSE_ARRAY]:
            return OptimizedBoolArrayNode(reader,codec).get_values(code)
        elif code in [TypeCodes.INT32_ZERO_ARRAY, TypeCodes.INT32_BYTE_ARRAY, TypeCodes.INT32_SHORT_ARRAY, TypeCodes.INT32_24BIT_ARRAY]:
            return OptimizedIntArrayNode(reader,codec).get_values(code)
        elif code in [TypeCodes.UINT32_BYTE_ARRAY,TypeCodes.UINT32_SHORT_ARRAY,TypeCodes.UINT32_24BIT_ARRAY]:
            return OptimizedUIntArrayNode(reader,codec).get_values(code)
        else:
            raise
