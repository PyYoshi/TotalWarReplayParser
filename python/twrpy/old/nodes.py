...
from twrpy.old.types import TypeCodes
...
class OptimizedBoolNode(BaseNode):
    type_code = TypeCodes.BOOL
    def get_value(self,code):
        if code == TypeCodes.BOOL_TRUE:
            return True
        elif code == TypeCodes.BOOL_FALSE:
            return False
        else:
            raise


class OptimizedIntNode(BaseNode):
    type_code = TypeCodes.INT32
    def get_value(self,code):
        if code == TypeCodes.INT32_ZERO:
            return 0
        elif code == TypeCodes.INT32_BYTE:
            return self.reader.read_int8_value()
        elif code == TypeCodes.INT32_SHORT:
            return self.reader.read_int16_value()
        elif code == TypeCodes.INT32_24BIT:
            return self.reader.read_int24_value()
        else:
            raise


class OptimizedUIntNode(BaseNode):
    type_code = TypeCodes.INT32
    def get_value(self,code):
        if code == TypeCodes.UINT32_ZERO:
            return 0
        elif code == TypeCodes.UINT32_ONE:
            return 1
        elif code == TypeCodes.UINT32_BYTE:
            return self.reader.read_uint8_value()
        elif code == TypeCodes.UINT32_SHORT:
            return self.reader.read_uint16_value()
        elif code == TypeCodes.UINT32_24BIT:
            return self.reader.read_uint24_value()
        else:
            raise


class OptimizedFloatNode(BaseNode):
    type_code = TypeCodes.FLOAT32
    def get_value(self,code):
        if code == TypeCodes.FLOAT32_ZERO:
            return 0
        else:
            raise
...
class BaseArrayNode(BaseNode):
    ...
    def get_read_size(self):
        ...
        while (cvalue & 0x80) != 0:
            ...
            cvalue = self.reader.read_uint8_value()
        return int((result << 7) + (cvalue & 0x7f))
    ...
    def get_offset(self):
        if self.codec.CODEC_NAME == 'ABCA':
            position = self.reader.index
            size = self.get_read_size()
            b = []
            for i in range(size):
                b.append(self.reader.read_int8_value())
            print b
            return position
        else:
            return self.reader.read_uint32_value()
...
class OptimizedBoolArrayNode(OptimizedBoolNode,BaseArrayNode):
    type_code = TypeCodes.BOOL
    def __init__(self,reader,codec):
        BaseArrayNode.__init__(self,reader,codec)
    ...
    def get_values(self,code):
        ...
        while True:
            if self.reader.index >= self.offset:
                ...
                self.reader.read_uint8_value()
                if code == TypeCodes.BOOL_TRUE_ARRAY:
                    values.append(True)
                elif code == TypeCodes.BOOL_FALSE_ARRAY:
                    values.append(False)
                else:
                    raise
        return values
...
class OptimizedIntArrayNode(OptimizedIntNode,BaseArrayNode):
    type_code = TypeCodes.INT32
    def __init__(self,reader,codec):
        BaseArrayNode.__init__(self,reader,codec)
    ...
    def get_values(self,code):
        ...
        while True:
            if self.reader.index >= self.offset:
                ...
                if code == TypeCodes.INT32_BYTE_ARRAY:
                    value = self.reader.read_int8_value()
                elif code == TypeCodes.INT32_SHORT_ARRAY:
                    value = self.reader.read_int16_value()
                elif code == TypeCodes.INT32_24BIT_ARRAY:
                    value = self.reader.read_int24_value()
                else:
                    raise
                values.append(value)
        return values
...
class OptimizedUIntArrayNode(OptimizedUIntNode,BaseArrayNode):
    type_code = TypeCodes.UINT32
    def __init__(self,reader,codec):
        BaseArrayNode.__init__(self,reader,codec)
    ...
    def get_values(self,code):
        ...
        while True:
            if self.reader.index >= self.offset:
                ...
                if code == TypeCodes.UINT32_BYTE_ARRAY:
                    value = self.reader.read_uint8_value()
                elif code == TypeCodes.UINT32_SHORT_ARRAY:
                    value = self.reader.read_uint16_value()
                elif code == TypeCodes.UINT32_24BIT_ARRAY:
                    value = self.reader.read_uint24_value()
                else:
                    raise
                values.append(value)
        return values

class OptimizedFloatArrayNode(OptimizedFloatNode,BaseArrayNode):
    type_code = TypeCodes.FLOAT32
    def __init__(self,reader,codec):
        BaseArrayNode.__init__(self,reader,codec)
    ...
    def get_values(self,code):
        ...
        while True:
            if self.reader.index >= self.offset:
                ...
                if code == TypeCodes.FLOAT32_ZERO_ARRAY:
                    values.append(0)
        return values
...
class RecordNode(object):
    ...
    def get_tag_name(self,index):
        return self.codec.FOOTER['tags'][index]
    ...
    def get_offset(self):
        if self.codec.CODEC_NAME in ["ABCE","ABCF"]:
            ...
            b = []
            for i in range(size):
                b.append(self.reader.read_uint8_value())
            print '============================='
            print size
            print b
            result = position
